...
class SatEdgeSimBackend:
    ...
    def get_planner_state(self, context: Any | None = None, scope: Any | None = None, budget: Any | None = None) -> PlannerState:
        ...
        return PlannerState(
            ...
            acquisition=MonitorAcquisitionMetadata(
                obs_bytes=len(json.dumps(state, default=str)),
                num_queries=1,
                source=source,
    ...
    def dispatch_under_configuration(self, configuration: PersistentConfiguration, task: Any | None = None) -> Mapping[str, Any]:
        action = self.materialize_current_configuration(configuration, task)
        if not isinstance(action, Mapping):
            raise SatEdgeSimCapabilityError("Persistent configuration has no materialized SatEdgeSim action")
        return self.client.apply_action(dict(action))
    ...
    def advance_world(self, delta_sec: float) -> Mapping[str, Any]:
        ...
        return self.client._request("POST", "/advance_world", json={"deltaSec": float(delta_sec)})

    def validate_configuration(self, configuration: PersistentConfiguration) -> bool:
        if not configuration.assignments and getattr(configuration, "covered_task_ids", set()):
            return False
        return True
    ...
    def _detect_capabilities(self) -> BackendCapabilities:
        ...
        return BackendCapabilities(
            ...
            supports_persistent_configuration=endpoints["/configuration/apply"],
    ...
    def _monitor_from_payload(self, state: Mapping[str, Any], *, source: str, true_cheap: bool) -> MonitorState:
        ...
        acquisition = MonitorAcquisitionMetadata(
            ...
            monitor_bytes=len(json.dumps(state, default=str)),
        )
        return MonitorState(
            simulation_time=self._extract_time(state),
