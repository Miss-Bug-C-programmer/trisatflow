...
## TriSatFlow Paper-Ready V3

This checkout contains the TriSatFlow paper-ready v3 repair pipeline. Use the explicit v3 entrypoint; the legacy `scripts/run_paper_ready_pipeline.sh` is deprecated and only forwards safe compatibility calls.
...
SatEdgeSim live preflight requires a running REST bridge and an explicit upstream root. The upstream path is not hardcoded in TriSatFlow.
...
Run formal GPU jobs only after the offline preflight, dry-run, and SatEdgeSim live preflight have written their `GATE_OK` artifacts. See `docs/paper_ready_v3/REPRODUCIBILITY.md` and `docs/paper_ready_v3/FORMAL_RUNBOOK.md`.
